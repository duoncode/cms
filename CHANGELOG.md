...
- **Changed** `Duon\Cms\Config` construction to `new Config(string $root, array $settings = [])`. App name, debug mode, and environment now live in `app.name`, `app.debug`, and `app.env` settings instead of constructor arguments or public properties. `app.name` is not validated or normalized.
