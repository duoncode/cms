...
- **Changed** CMS session options to read `cookie_secure` from `CMS_SECURE_COOKIE`, `cookie_lifetime` from `CMS_SESSION_COOKIE_LIFETIME`, and `gc_maxlifetime` from `CMS_SESSION_IDLE_TIMEOUT`.
